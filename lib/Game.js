...
module.exports = class Game {
  ...
  animate() {
    ...
    this.blocks.forEach( block => {
      ...
      //create separate player trails
      const blueTrail = this.path.filter(pathUnit => pathUnit.color === 'cornflowerblue');
      console.log(blueTrail)
      const orangeTrail = this.path.filter(pathUnit => pathUnit.color === 'orange');
  ...
  handleBlock(block, pathUnit) {
    ...
    if (block.isCollidingWithWall(canvas.width, canvas.height)) {
        this.endGame(); 
      // } else if (block.isCollidingWith(pathUnit)) {
        // this.endGame();
      }  else {
        block.move();
      }
